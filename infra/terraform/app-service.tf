...
output "api_url" {
  value = !var.app_service_exists ? "https://${azurerm_linux_web_app.api[0].default_hostname}" : "No new App Service created"
}

# Output the App Service name
output "app_service_name" {
  value = !var.app_service_exists ? azurerm_linux_web_app.api[0].name : (var.app_service_exists ? "${var.app_name}-api" : "No App Service")
}
